...
def writeToFile(df, dirname, fname):
    df["date"]= pd.to_datetime(df["date"])
    inc = df.close > df.open
    dec = df.open > df.close
    w = 12*60*60*1000 # half day in ms
    ...
    p.vbar(df.date[inc], w, df.open[inc], df.close[inc], fill_color="#D5E1DD", line_color="black")
    p.vbar(df.date[dec], w, df.open[dec], df.close[dec], fill_color="#F2583E", line_color="black")
    bokeh.io.export_png(p, filename = dirname + fname + ".png")
def sliceCrypto(length, crypto):
    path = "C:/CryptoCNN/pricedatabase/"+crypto+"_USD.csv"
    df = pd.read_csv(path, header = 0, sep = ',', parse_dates = True)
    for i in range(0,  int(5 * df.count()[1]/length) ):
        temp = df[int((length/5)*i):min(len(df),int((length/5)*i + length))]
        writeToFile(temp, "actualplots/", crypto+"_" + str(temp['date'].iloc[0]) + "-" + str(temp['date'].iloc[len(temp)-1]))
